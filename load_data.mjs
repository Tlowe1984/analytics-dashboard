...
const DATABASE_URL = rawDbUrl.replace(/[?&]ssl=[^&]*/g, '').replace(/\?$/, '').replace(/\?&/, '?');

const connection = await mysql.createConnection({ uri: DATABASE_URL, ...(isLocal ? {} : { ssl: { rejectUnauthorized: true } }) });
